...
def _normalize_nodata_for_whitebox(input_tif: str, output_tif: str, nodata_value: float = -9999.0) -> float:
    ...
    # Se o nodata já é numérico e não há NaNs, apenas replica
    ...
    # Caso comum no pipeline: nodata NaN
...
def realizar_analise_de_fluxo(cidade: str):
    ...
    if not mde_in.exists():
        raise FileNotFoundError(f"MDE não encontrado: {mde_in}. Execute _processar_mde antes.")

    # Saída final (no workspace)
    ...
    with tempfile.TemporaryDirectory(prefix=f"wbt_fluxo_{cidade}_") as tmp:
        ...
        _normalize_nodata_for_whitebox(str(tmp_mde_in), str(tmp_mde_nodata), nodata_value=-9999.0)
