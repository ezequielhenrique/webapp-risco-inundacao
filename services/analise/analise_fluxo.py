...
class AnaliseFluxo(AnaliseBase):
    ...
    def executar(self):
        ...
        mde_in = Path(f"outputs/mde/mde_{self.cidade}.tif")
        out_acc = out_dir / f"fluxo_acumulado_{self.cidade}.tif"
        ...
        with tempfile.TemporaryDirectory(prefix=f"wbt_fluxo_{self.cidade}_") as tmp:
            tmp_dir = Path(tmp)
            ...
            self._normalize_nodata_for_whitebox(str(tmp_mde_in), str(tmp_mde_nodata), nodata_value=-9999.0)
